...
public class ValidateTermAndTaxesListener implements TaskListener {
    ...
    public void notify(DelegateTask delegateTask) {
        ...
        DocumentContext documentContext = JsonPath.using(jsonPathConfiguration).parse(processContext);
        String projectId = documentContext.read("$.formioBaseProject");
        String arId = documentContext.read("$.service.data.arId");
        String supplierId = documentContext.read("$.serviceSupplier.data.code");
        ...
        try {
            ...
            throw new EFormsProcessEngineException(SeverityEnum.WARN, "CHANNEL_AND_SERVICE_TYPE_NOT_FOUND");
        }
    }
