...
export default function ChartRenderer({
  ...
  const renderExtras = () => {
    ...
    if (queryHasMissingMembers) {
      return (
        <div>
          {t("At least one of the query members is missing from your data schema.")}
          {t("Please update your query or data schema.")}
        </div>
      );
    }
  ...
  const slowQueryMsg = slowQuery
    ? t('This query takes more than 5 seconds to execute. Please consider using pre-aggregations to improve its performance. ')
    : slowQueryFromCache
    ? t("This query takes more than 5 seconds to execute. It was served from the cache because Cube.js wasn't able to renew it in less than 5 seconds. Please consider using pre-aggregations to improve its performance. ")
    : '';
