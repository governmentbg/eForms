  ...
  async function handleAddToSchemaClick() {
    ...
    function showSuccessMessage() {
      notification.success({
        message: `${t("Rollup has been added to the")} ${cubeName} ${t("cube")}`,
      });
    }
      ...
      return (
        <Paragraph>
          <Link
            ...
            target="_blank"
          >
            {t("Current query cannot be rolled up due to it is not additive")}
          </Link>
          . {t("Please consider removing not additive measures like `countDistinct` or `avg`. You can also try to use")}{' '}
          <Link
    ...
    return (
      <>
        <CodeSnippet
          ...
          copyMessage={t("Rollup definition is copied")}
          theme="light"
        />
    ...
    <Wrapper>
      <MainBox grow={1} style={{ overflowX: 'scroll' }}>
        <Tabs style={{ minHeight: '100%' }}>
          <TabPane
            tab={<span data-testid="rd-members-tab">{t("Members")}</span>}
            key="members"
          >
            <Flex gap={2}>
              <Box
          ...
          <TabPane
            tab={<span data-testid="rd-settings-tab">{t("Settings")}</span>}
            key="settings"
          >
            <Settings
      ...
      <RollupQueryBox>
        <Tabs>
          <TabPane
            tab={<span data-testid="rd-definition-tab">{t("Rollup Definition")}</span>}
            key="rollup"
          >
            <Flex direction="column" justifyContent="flex-start">
              {!areReferencesEmpty(references) &&
                !references.timeDimensions.length && (
                  <Box style={{ marginBottom: 24 }}>
                    <Alert
                      type="warning"
                      message={t("This rollup has no time dimension so it cannot be partitioned")}
                    />
                  </Box>
                )}
              ...
              {canBeRolledUp ? (
                <Box style={{ marginBottom: 16 }}>
                  {!areReferencesEmpty(references) ? (
                    <Paragraph>
                      {t("Add the following rollup pre-aggregation")}
                      <br /> {t("to the")} <b>{cubeName}</b> {t("cube")}:
                    </Paragraph>
                  ) : (
                    <Alert type="warning" message={t("Add some references")} />
                  )}
            ...
            <TabPane
              tab={
                canBeRolledUp && matching ? (
                  <span data-testid="rd-query-tab">{t("Query Compatibility")}</span>
                ) : (
                  <Typography.Text data-testid="rd-query-tab">
                    {t("Query Compatibility")}
            ...
            >
              <Flex direction="column" justifyContent="flex-start">
                <Box style={{ marginBottom: 32 }}>
                  {canBeRolledUp && matching ? (
                    <Text>{t("This rollup will match the following query")}:</Text>
                  ) : (
                    <Space direction="vertical">
                      <Alert
                        ...
                        message={
                          <Text>
                            {t("This rollup does NOT match the following query")}:
                          </Text>
                        }
                      />
