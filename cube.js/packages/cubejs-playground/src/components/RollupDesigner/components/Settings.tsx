...
export function Settings({
  ...
  return (
    ...
    >
      <Wrapper>
        <Card>
          <TitleWithTooltip title={t("Refresh Key")}>
            {t("Specify how often to refresh your pre-aggregated data")}
          </TitleWithTooltip>

          <Typography.Paragraph>
            {t("If you do not specify any Refresh Key, refreshes will still default to every 1 hour.")}
          </Typography.Paragraph>
                    ...
                    {isCron && values['refreshKey.checked.every'] ? (
                      <>
                        <Typography.Paragraph>
                          ...
                          >
                            {t("See how to format your cron expression")}
                          </Typography.Link>
                        </Typography.Paragraph>
        ...
        <Card>
          {hasTimeDimension ? (
            <>
              <TitleWithTooltip title={t("Partition Granularity")}>
                {t("Partitions are shards of the pre-aggregation dataset. To enable partitions, you must specify here the desired granularity.")}
              </TitleWithTooltip>
              ...
              {values.partitionGranularity ? (
                <>
                  <Form.Item name="incrementalRefresh" valuePropName="checked">
                    <Checkbox>{t("Incremental Refresh")}</Checkbox>
                  </Form.Item>

                  {values['incrementalRefresh'] && (
                    <>
                      <TitleWithTooltip title={t("Update Window")}>
                        {t("Any partition which includes this span of time into the past from now will be refreshed according to the Refresh Key set above. Otherwise, if left unset, only the most recent partition will be refreshed regularly.")}
                      </TitleWithTooltip>
  ...
  <BuildRange time="until" />
          ...
          <Typography.Paragraph strong>{t("Indexes")}</Typography.Paragraph>
