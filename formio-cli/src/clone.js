...
module.exports = function(source, destination, options) {
  ...
  MongoClient.connect(source, srcConfig, (err, _src) => {
    ...
    MongoClient.connect(destination, dstConfig, (err, _dest) => {
      ...
      const getFormQuery = function(query) {
        if (options.tags) {
          query.tags = {$all: options.tags.split(',')};
        }
        return query;
      };
      ...
      if (options.submissionsOnly) {
        ...
        dest.projects.findOne({name: 'formio'}, (err, formioProject) => {
          ...
          upsertAll('projects', srcProjectQuery({...itemQuery}), (project) => {
            ...
            upsertAll('forms', getFormQuery({...itemQuery}), (form) => {
              ...
              let skipSubmissions = false;
              upsertAll('actions', getItemQuery(itemQuery), (action) => {
                  action.form = clonedForm._id;
                }, null, () => {
                  if (options.excludeSubmissionsTags) {
                    process.stdout.write("\n");
                    process.stdout.write(`   - Check excluded tags: : `);
                    options.excludeSubmissionsTags.split(",").forEach(excludedTag => {
                      if (form.tags.indexOf(excludedTag) > -1) {
                        process.stdout.write(`Found: ${excludedTag} skipping submissions `);
              ...
              upsertAll('roles', itemQuery, (role) => {
                ...
                upsertAll('tags', getItemQuery(itemQuery), (tag) => {
                  ...
                  upsertAll('formrevisions', getFormQuery(itemQuery), (revision) => {
                    process.stdout.write("\n");
