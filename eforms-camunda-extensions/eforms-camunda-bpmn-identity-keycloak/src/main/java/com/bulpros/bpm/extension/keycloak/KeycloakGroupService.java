package com.bulpros.bpm.extension.keycloak;

import static org.camunda.bpm.engine.authorization.Permissions.READ;
import static org.camunda.bpm.engine.authorization.Resources.GROUP;
import static com.bulpros.bpm.extension.keycloak.json.JsonUtil.*;
...
import com.bulpros.bpm.extension.keycloak.json.JsonException;
import com.bulpros.bpm.extension.keycloak.rest.KeycloakRestTemplate;
import com.bulpros.bpm.extension.keycloak.util.KeycloakPluginLogger;
...
public class KeycloakGroupService extends KeycloakServiceBase {
		...
		try {
			try {
				ResponseEntity<String> response = restTemplate.exchange(
						keycloakConfiguration.getKeycloakAdminUrl() + "/roles?search=" + configuredAdminGroupName, HttpMethod.GET, String.class);
				if (keycloakConfiguration.isUseGroupPathAsCamundaGroupId()) {
					var result  = response.getBody();
					JsonArray array = (JsonArray) JsonParser.parseString(result);
					return parseAsJsonObjectAndGetMemberAsString(array.get(0).toString(), "name"); // remove trailing '/'
				}
	...
	public List<Group> requestGroupsByUserId(CacheableKeycloakGroupQuery query) {
		...
		try {
			...
			ResponseEntity<String> response = restTemplate.exchange(
					keycloakConfiguration.getKeycloakAdminUrl() + "/users/" + keyCloakID + "/role-mappings/realm/composite?max=" + getMaxQueryResultSize(),
	...
	private GroupEntity transformGroup(JsonObject result) throws JsonException {
		...
		if (keycloakConfiguration.isUseGroupPathAsCamundaGroupId()) {
			group.setId(getJsonString(result, "name")); // remove trailing '/'
		} else {
