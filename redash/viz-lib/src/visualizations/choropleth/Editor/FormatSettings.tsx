  ...
  return (
    ...
    <ContextHelp placement="topLeft" arrowPointAtCenter>
      <div style={{ paddingBottom: 5 }}>
        <div>
          {t("All query result columns can be referenced using")} <code>{"{{ column_name }}"}</code> {t("syntax.")}
        </div>
        <div>
          {t("Use")} <code>{"{{ @@value }}"}</code> {t("to access formatted value.")}
        </div>
      </div>
      {geoJsonProperties.length > 0 && (
        <React.Fragment>
          <div className="p-b-5">{t("GeoJSON properties could be accessed by these names:")}</div>
