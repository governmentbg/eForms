  ...
  const queryHint =
    query && query.schedule ? (
      ...
      <small>
        ...
        <Tooltip title={t("A query schedule is not necessary but is highly recommended for alerts. An Alert without a query schedule will only send notifications if a user in your organization manually executes this query.")}>
          <a role="presentation">
            {t("Why it's recommended")} <QuestionCircleTwoToneIcon />
          </a>
        </Tooltip>
      </small>
    );
    ...
    <>
      ...
      {query && !queryResult && (
        <div className="m-t-30">
          <LoadingOutlinedIcon className="m-r-5" /> {t("Loading query data")}
        </div>
      )}
    </>
  );
