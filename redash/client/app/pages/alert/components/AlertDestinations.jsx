...
export default class AlertDestinations extends React.Component {
  ...
  showAddAlertSubDialog = () => {
    ...
    SelectItemsDialog.showModal({
      ...
      extraFooterContent: (
        <>
          <i className="fa fa-info-circle" aria-hidden="true" /> {t("Create new destinations in")}{" "}
          <Tooltip title={t("Opens page in a new tab.")}>
            <Link href="destinations/new" target="_blank">
              {t("Alert Destinations")}
            </Link>
          </Tooltip>
        </>
      ),
      dialogTitle: t("Add Existing Alert Destinations"),
      inputPlaceholder: t("Search destinations..."),
      ...
      return Promise.all(promises)
        .then(() => {
          notification.success(t("Subscribed."));
        })
        .catch(() => {
          notification.error(t("Failed saving subscription."));
  ...
  render() {
    ...
    return (
      <div className="alert-destinations" data-test="AlertDestinations">
        <Tooltip title={t('Click to add an existing "Alert Destination"')} mouseEnterDelay={0.5}>
          <Button
            ...
            <i className="fa fa-plus f-12 m-r-5" aria-hidden="true" /> {t("Add")}
          </Button>
        </Tooltip>
        <ul>
          <li className="destination-wrapper">
