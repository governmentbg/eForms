    ...
    <div className="alert-rearm">
      <Select
        ...
        <Select.Option value={0} label={t("Just once")}>
          {t("Just once")} <em>{t("until back to normal")}</em>
        </Select.Option>
        <Select.Option value={1} label={t("Each time alert is evaluated")}>
          {t("Each time alert is evaluated")} <em>{t("until back to normal")}</em>
        </Select.Option>
        <Select.Option value={2} label={t("At most every")}>
          {t("At most every")} ... <em>{t("when alert is evaluated")}</em>
        </Select.Option>
      </Select>
...
function RearmViewer({ value }) {
  ...
  switch (value) {
    ...
    default:
      phrase = (
        <>
          {t("at most every")} <RearmByDuration value={value} editMode={false} />, {t("when alert is evaluated")}
        </>
      );
  }

  return <span>{t("Notifications are sent")} {phrase}.</span>;
}
