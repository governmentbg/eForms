...
function RefreshButton({ dashboardConfiguration }) {
  ...
  return (
    <Button.Group>
      <Tooltip title={refreshRate ? `${t("Auto Refreshing every")} ${durationHumanize(refreshRate)}` : null}>
        <Button type={buttonType(refreshRate)} onClick={() => refreshDashboard()}>
          ...
          {refreshRate ? t(durationHumanize(refreshRate)) : t("Refresh")}
        </Button>
      </Tooltip>
      <Dropdown
        trigger={["click"]}
      ...
      {!dashboard.is_archived && (
        <span className="hidden-print">
          ...
          {showFullscreenButton && (
            <Tooltip className="hidden-xs" title={t("Enable/Disable Fullscreen display")}>
              <Button
...
function DashboardEditControl({ dashboardConfiguration, headerExtra }) {
  ...
  if (dashboardStatus === DashboardStatusEnum.SAVED) {
    status = <span className="save-status">{t("Saved")}</span>;
    ...
    <div className="dashboard-control">
      ...
      {dashboardStatus === DashboardStatusEnum.SAVING_FAILED ? (
        ...
        <Button loading={doneBtnClickedWhileSaving} type="primary" onClick={() => setEditingLayout(false)}>
          {!doneBtnClickedWhileSaving && <i className="fa fa-check m-r-5" aria-hidden="true" />} {t("Done Editing")}
        </Button>
      )}
      {headerExtra}
    </div>
