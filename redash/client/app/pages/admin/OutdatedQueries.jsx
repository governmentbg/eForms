...
class OutdatedQueries extends React.Component {
  ...
  listColumns = [
    ...
    Columns.dateTime.sortable({ title: t("Created At"), field: "created_at" }),
    Columns.duration.sortable({ title: t("Runtime"), field: "runtime" }),
    Columns.dateTime.sortable({ title: t("Last Executed At"), field: "retrieved_at", orderByField: "executed_at" }),
      ...
      <Layout activeTab={controller.params.currentPage}>
        <div className="m-15">
          ...
          {controller.params.lastUpdatedAt && (
            <div className="m-t-5">
              {t("Last updated:")} <TimeAgo date={controller.params.lastUpdatedAt * 1000} />
            </div>
          )}
        </div>
        {!controller.isLoaded && <LoadingState />}
        {controller.isLoaded && controller.isEmpty && (
          <div className="text-center p-15">{t("There are no outdated queries.")}</div>
        )}
        {controller.isLoaded && !controller.isEmpty && (
