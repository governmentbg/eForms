...
class CreateSourceDialog extends React.Component {
  ...
  render() {
    ...
    let { dialog, sourceType } = this.props;
    ...
    return (
      <Modal
        {...dialog.props}
        title={`${t("Create a New")} ${sourceType}`}
        footer={
          currentStep === StepEnum.SELECT_TYPE
            ? [
        ...
        <div data-test="CreateSourceDialog">
          <Steps className="hidden-xs m-b-10" size="small" current={currentStep} progressDot>
            {currentStep === StepEnum.CONFIGURE_IT ? (
              <Step title={<a>{t("Type Selection")}</a>} className="clickable" onClick={this.resetType} />
            ) : (
              <Step title={t("Type Selection")} />
            )}
            <Step title={t("Configuration")} />
            <Step title={t("Done")} />
          </Steps>
