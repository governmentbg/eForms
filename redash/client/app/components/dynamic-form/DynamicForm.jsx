...
const fieldRules = ({ type, required, minLength }) => {
  ...
  return [
    requiredRule && { required, message: t("This field is required.") },
    minLengthRule && { min: minLength, message: t("This field is too short.") },
    emailTypeRule && { type: "email", message: t("This field must be a valid email.") },
  ].filter(rule => rule);
};
...
function DynamicFormFields({ fields, feedbackIcons, form }) {
  return fields.map(field => {
    ...
    const formItemProps = {
      ...
      label: type === "checkbox" ? "" : t(fieldLabel),
