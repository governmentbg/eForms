...
class ScheduleDialog extends React.Component {
  ...
  render() {
    ...
    return (
      <Modal {...dialog.props} title={t("Refresh Schedule")} className="schedule" onOk={() => this.save()} okText={t("Save")} cancelText={t("Cancel")}>
        <div className="schedule-component">
          <h5>{t("Refresh every")}</h5>
          <div data-testid="interval">
            <Select className="input" value={seconds} onChange={this.setInterval} dropdownMatchSelectWidth={false}>
              ...
              {Object.keys(this.intervals)
                ...
                .map(int => (
                  <OptGroup label={t(capitalize(pluralize(int)))} key={int}>
                    {this.intervals[int].map(([cnt, secs]) => (
                      <Option value={secs} key={`${int}-${cnt}`}>
                        {t(durationHumanize(secs))}
                      </Option>
                    ))}
                  </OptGroup>
                ))}
          ...
          <div className="schedule-component">
            ...
            <div className="ends" data-testid="ends">
              <Radio.Group size="medium" value={!!until} onChange={this.setUntilToggle}>
                <Radio value={false}>{t("Never")}</Radio>
                <Radio value>{t("On")}</Radio>
              </Radio.Group>
              {until ? (
                <DatePicker
                  size="small"
