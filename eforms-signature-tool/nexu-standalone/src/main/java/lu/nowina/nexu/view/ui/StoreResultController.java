...
public class StoreResultController extends AbstractFeedbackUIOperationController implements Initializable {
	...
	private static final Logger logger = LogManager.getLogger(StoreResultController.class.getName());
	...
	protected void doInit(Object... params) {
		...
		Platform.runLater(() -> {
			...
			message.setText(StringEscapeUtils.unescapeJava(MessageFormat.format(
					Utf8ResourceBundle.getUTF8Bundle("bundles/nexu").getString("store.configuration.header"),
					getApplicationName())));
		}
		);
	}
