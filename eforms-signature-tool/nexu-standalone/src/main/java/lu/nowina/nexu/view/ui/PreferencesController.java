...
public class PreferencesController extends AbstractUIOperationController<Void> implements Initializable {
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		languages.getItems().setAll("English","Bulgarian");
		...
		ok.setOnAction((evt) -> {
			...
			if (!StringUtil.isBlank(languages.getValue())){
				...
				String bundleName = StageHelper.getBundleName();
				StageHelper.getInstance().setBundle(Utf8ResourceBundle.getUTF8Bundle(bundleName));
				SystrayMenu.reloadLabels();
			}
