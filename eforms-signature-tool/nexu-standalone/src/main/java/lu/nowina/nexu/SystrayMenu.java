...
public class SystrayMenu {
	...
	private static final Logger LOGGER = LogManager.getLogger(SystrayMenu.class.getName());
	private static ResourceBundle resources;
	private static SystrayMenuInitializer menuInitializer;
	private static SystrayMenuItem[] systrayMenuItems;
	private static SystrayMenuItem exitMenuItem;

	public SystrayMenu(OperationFactory operationFactory, NexuAPI api, UserPreferences prefs) {
		resources = Utf8ResourceBundle.getUTF8Bundle("bundles/nexu");

		final List<SystrayMenuItem> extensionSystrayMenuItems = api.getExtensionSystrayMenuItems();
		systrayMenuItems = new SystrayMenuItem[extensionSystrayMenuItems.size() + 2];

		systrayMenuItems[0] = createAboutSystrayMenuItem(operationFactory, api);
		...
		exitMenuItem = createExitSystrayMenuItem();
		...
		try {
			...
			case MACOSX:
				...
				menuInitializer = Class.forName("lu.nowina.nexu.systray.AWTSystrayMenuInitializer")
					.asSubclass(SystrayMenuInitializer.class).newInstance();
					menuInitializer.init(tooltip, trayIconURL, operationFactory, exitMenuItem, systrayMenuItems);
				break;
			case LINUX:
				// Use reflection to avoid wrong initialization issues
				menuInitializer = Class.forName("lu.nowina.nexu.systray.DorkboxSystrayMenuInitializer")
					.asSubclass(SystrayMenuInitializer.class).newInstance();
					menuInitializer.init(tooltip, trayIconURL, operationFactory, exitMenuItem, systrayMenuItems);
				break;
			case NOT_RECOGNIZED:
	...
	private static SystrayMenuItem createAboutSystrayMenuItem(final OperationFactory operationFactory, final NexuAPI api) {
	...
	public static void reloadLabels(){
		...
		for (SystrayMenuItem systrayMenuItem : systrayMenuItems) {
			labels.add(systrayMenuItem.getLabel());
		}
		labels.add(exitMenuItem.getLabel());
		menuInitializer.reloadMenuItemLabels(labels);
	}
}
