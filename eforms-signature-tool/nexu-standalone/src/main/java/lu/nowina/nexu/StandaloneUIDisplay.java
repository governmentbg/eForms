...
public class StandaloneUIDisplay implements UIDisplay {
	...
	private static final Logger LOGGER = LogManager.getLogger(StandaloneUIDisplay.class.getName());
	...
	private final class FlowPasswordCallback implements NexuPasswordInputCallback {
		...
		public char[] getPassword() {
			...
			Optional<char[]> currentPassword = PasswordCache.getPassword(this.toString());
			if (currentPassword.isPresent()) {
				...
				final OperationResult<char[]> passwordResult = StandaloneUIDisplay.this.operationFactory.getOperation(
						UIOperation.class, "/fxml/password-input.fxml", passwordPrompt, NexuLauncher.getConfig().getApplicationName(), this.toString()).perform();
	...
	public File displayFileChooser(ExtensionFilter... extensionFilters) {
		...
		fileChooser.setTitle(Utf8ResourceBundle.getUTF8Bundle("bundles/nexu").getString("fileChooser.title.openResourceFile"));
