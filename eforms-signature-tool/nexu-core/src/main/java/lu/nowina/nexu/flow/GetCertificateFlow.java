...
class GetCertificateFlow extends AbstractCoreFlow<GetCertificateRequest, GetCertificateResponse> {
    ...
    static final Logger logger = LogManager.getLogger(GetCertificateFlow.class);
    	...
    	try {
    		...
    		while (true) {
    			...
    			if(defaultProduct != null) {
					...
					List<Product> detectProducts = new ArrayList<>(); // api.detectProducts();
					List<DetectedCard> detectCards = api.detectCards();
					if (detectCards.size() != 1) {
						final Object[] params = {
								api.getAppConfig().getApplicationName(), api.detectCards(), detectProducts, api
						};
						...
						selectedProduct = detectCards.get(0);
					}
    			}
