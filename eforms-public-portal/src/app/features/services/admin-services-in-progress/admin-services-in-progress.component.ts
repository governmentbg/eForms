...
export class AdminServicesInProgressComponent implements OnInit {
  ...
  handleFormReady(form: any) {
    this.form = form;
    this.parameters = `&assignedToMe=${this.form.formio.data.assignedToMeFilter}`
    this.filterTable();
  }
  ...
  handleFilterEvent(event) {
    if (event.type === 'filter') {
      ...
      if (event.data.serviceDropdown.serviceId) {
        serviceId= `&serviceId=${event.data.serviceDropdown.serviceId}`
      }
      this.parameters = `${serviceId}&businessKey=${event.data.uinFilter}&requestorName=${event.data.requestorNameFilter}&assignedToMe=${event.data.assignedToMeFilter}&fromIssueDate=${fromDate}&toIssueDate=${toDate}`
      this.filterTable();
    }
  }
